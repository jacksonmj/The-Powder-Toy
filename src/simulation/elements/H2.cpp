...
int H2_update(UPDATE_FUNC_ARGS)
{
	...
	for (rx=-2; rx<3; rx++)
		for (ry=-2; ry<3; ry++)
			if (x+rx>=0 && y+ry>=0 && x+rx<XRES && y+ry<YRES && (rx || ry))
			{
				...
				{
					rt = parts[ri].type;
					if (sim->air.pv.get(SimPosI(x,y)) > 8.0f && rt == PT_DESL) // This will not work well. DESL turns to fire above 5.0 pressure
					{
						sim->part_change_type(ri,x+rx,y+ry,PT_WATR);
						sim->part_change_type(i,x,y,PT_OIL);
						typeChanged = true;
					}
					if (sim->air.pv.get(SimPosI(x,y)) > 45.0f)
					{
	...
	if (parts[i].temp > 2273.15 && sim->air.pv.get(SimPosI(x,y)) > 50.0f)
	{
