	...
	for (rx=-1; rx<2; rx++)
		for (ry=-1; ry<2; ry++)
			if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
			{
				...
				{
					rt = parts[ri].type;
					if ((sim->elements[rt].Properties&TYPE_SOLID) && sim->rng.chance<1,10>() && parts[i].life==0 && !(rt==PT_CLNE||rt==PT_PCLN)) // TODO: should this also exclude BCLN?
					{
						sim->part_change_type(i,x,y,PT_RIME);
					}
					if (rt==PT_SPRK)
					{
						parts[i].life += sim->rng.randInt<0,19>();
					}
				}
			}
	return 0;
