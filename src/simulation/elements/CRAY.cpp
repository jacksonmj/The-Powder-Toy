					...
					FOR_PMAP_POSITION_NOENERGY(sim, x+rx, y+ry, rcount, ri, rnext)
					{
						if (parts[ri].type==PT_SPRK && parts[ri].life==3)
						{
							...
							for (docontinue = 1, nxx = 0, nyy = 0, nxi = -rx, nyi = -ry; docontinue; nyy+=nyi, nxx+=nxi) {
								...
								if (!(sim->walls.type(SimPosI(x+nxi+nxx, y+nyi+nyy)) && sim->isWallBlocking(SimPosI(x+nxi+nxx, y+nyi+nyy), parts[i].ctype)) && (!sim->pmap[y+nyi+nyy][x+nxi+nxx].count(PMapCategory::Plain) || createSpark)) { // create, also set color if it has passed through FILT
									int nr;
