...
void fillrect(unsigned *vid, int x, int y, int w, int h, int r, int g, int b, int a)
{
	int i,j;
	for (j=0; j<h; j++)
		for (i=0; i<w; i++)
...
int main(int argc, char *argv[])
{
	...
	while (!sdl_poll()) {
		...
		if (b && !lb && x>=CELLW*32+200 && x<CELLW*32+200+CELLW*16 && y>=50 && y<50+CELLH*16*3/2)
		{
		...
		if (x>=CELLW*32+200 && x<CELLW*32+200+CELLW*16 && y>=50 && y<50+CELLH*16*3/2)
		{
			int hc = (x-(CELLW*32+200))/CELLW + (y-50)*2/(3*CELLH)*16;
			fillrect(vid_buf, CELLW*32+200+CELLW*(hc%16), 50+CELLH*(hc/16)*3/2, CELLW, CELLH, 255, 255, 255, 50);
		}
		for (i=0; i<256; i++)
		{
			if (i<16) {
				char txt[2];
				sprintf(txt, "%X", i);
				drawtext(vid_buf, CELLW*32+200+CELLW*(i%16), 50-20+CELLH*(i/16)*3/2, txt, 255, 0, 0);
			}
			if (!(i%16)) {
				char txt[2];
				sprintf(txt, "%X", i/16);
				drawtext(vid_buf, CELLW*32+200-20+CELLW*(i%16), 50+CELLH*(i/16)*3/2, txt, 255, 0, 0);
			}
			drawchar(vid_buf, CELLW*32+200+CELLW*(i%16), 50+CELLH*(i/16)*3/2, i, 255, 255, 255);
		}
		sprintf(selected_txt,"Current char: %02X", c);
		drawtext(vid_buf, CELLW*32+200, 10, selected_txt, 255, 255, 255);
